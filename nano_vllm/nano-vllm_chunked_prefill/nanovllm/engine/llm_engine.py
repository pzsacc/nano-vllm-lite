...
class LLMEngine:
    ...
    def step(self):
        ...
        # 细分统计当前 Batch 中 prefill 和 decode 的 tokens 数量
    ...
    def generate(
        ...
        while not self.is_finished():
            t = perf_counter()
            output, prefill_toks, decode_toks = self.step()
            dt = perf_counter() - t

            # 计算混合批次的吞吐
            prefill_throughput = prefill_toks / dt if prefill_toks > 0 else 0
            decode_throughput = decode_toks / dt if decode_toks > 0 else 0
